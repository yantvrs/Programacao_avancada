...
//2)Desenhar um elipsóide com centre coincidente com o centro do paralelepípedo com raios determinados pelo usuário;
...
int main() {
  ...
  //Elipsóide
  //Raio a 
  printf("Agora, informe as dimensões dos raios do elipsóide... \n");
  printf("A: ");
  scanf("%d",&a);
  ///Raio b
  printf("B: ");
  ...
  //Alocando elipsóide no paralelepípedo
  for(k=0;k<np;k++){
    for(i=0;i<nl;i++){
      for(j=0;j<nc;j++){
        ...
        if(elips<=1){
          cubo[k][i][j]=1; //Para construir o elipsóide
        }
        else{
